...
## The illustrated cut (current film)

Since 2026-09-22 the shipped tour is **drawn, not recorded**: every scene is an
SVG in the same hand-built style as the README hero (slate palette, lucide
glyphs, app-accurate chrome), rasterised and assembled with crossfades.

```bash
python3 scripts/demo-illustrate.py video   # -> build/film/nebula-tour.mp4 (1920x1080, ~72s)
python3 scripts/demo-illustrate.py svg     # scenes only, for stills
python3 scripts/demo-illustrate.py poster  # -> docs/assets/nebula-tour-poster.svg
```

**Why illustrated.** A recorded cut goes stale the moment the UI moves, needs a
live server, fixtures, a kernel *and* a real SLURM cluster for the capstone, and
can't be regenerated by anyone but the person holding that setup. The drawn cut
regenerates from source in seconds, stays on-message, and reruns clean whenever
the product changes — the screen-recorded scenes below remain the reference for
capturing individual README GIFs.

| # | Scene | Beat | Feature |
|---|---|---|---|
| 1 | Title | *The notebook your agent can drive — while you keep working in it.* | positioning |
| 2 | Money shot | You edit cell 2 while the agent rewrites cell 5; the conflict lands as a blue notice, not a red error | OCC sessions, presence rings, agent terminal |
| 3 | Driving context | Two notebooks open — the agent resolves "this cell" against the one you're **looking at** | `nebula context`, act-time drift notices |
| 4 | Fix with agent | A cell throws `KeyError`; one click, the agent reads, edits, re-runs | per-cell agent prompts, MCP/CLI |
| 5 | Directional runs | Run All ▾ → *Run cells above* / *Run cell and below*, anchored on the selection | split Run button, command palette |
| 6 | Collapse long code | A 200-line cell folds to a height you drag; the toolbar sticks as you scroll | `source_collapsed`/`source_height`, sticky toolbar |
| 7 | Time travel | History rail, a preview 3 steps back, **Restore here** | journaled undo, `HistoryPanel` |
| 8 | Search & replace | Regex find/replace across the notebook; panel toggles | `NotebookSearch`, `⌘⇧H` / ``Ctrl+` `` |
| 9 | Sealed evidence | A result replayed in a fresh kernel, outputs matched, notebook + artifacts + env hash-bound | replay seals, provenance store |
| 10 | Security | Passkeys, TOTP, and agent tokens that don't depend on tunnel topology | WebAuthn, `NEBULA_TOKEN` |
| 11 | Compute | Queue table with **per-queue arch**; an aarch64 queue hands its setup to an agent, then comes up online | SLURM allocations, multi-arch, ARM setup prompt |
| 12 | Get started | Two commands, end card | — |

Each scene is a function in `scripts/demo-illustrate.py`; add a feature by adding
one and listing it in `build_scenes()`. Keep captions to one claim, in the
product's voice, with the mechanism in the subtitle.

---

## Pre-production checklist (screen-recorded cut)

The original live-capture storyboard follows. It stays here as the reference for
shooting README GIFs and for any future recorded film.
